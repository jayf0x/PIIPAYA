...
async function getLatestDmgUrl(owner: string, repo: string) {
  const response = await fetch(`https://api.github.com/repos/${owner}/${repo}/releases/latest`)
  const release = await response.json()
  const dmgAsset = release.assets.find((asset: { name: string }) => asset.name.endsWith(".dmg"))
  return dmgAsset ? (dmgAsset.browser_download_url as string) : ""
}
...
export const HeroSection = () => {
  ...
  useEffect(() => {
    if (!downloadURL) {
      getLatestDmgUrl("jayF0x", "PIIPAYA").then(setDownloadURL)
    }
  }, [])
  ...
  return (
    <section className="min-h-screen flex flex-col items-center justify-center px-6 pt-16 pb-12 relative gap-12">
      ...
      >
        ...
        >
          AI-assisted anonymization that never leaves your machine. Strip PII from text and
          documents — offline, fast.
        </motion.p>
